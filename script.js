...
async function exportPDF() {
  ...
  try {
    ...
    const canvas = await html2canvas(document.body, {
      ...
      scale: 2,
      useCORS: true,
      ignoreElements: el => el.id === 'export-pdf'
    });
    ...
    if (imgH <= pageH) {
      ...
      while (offsetY < canvas.height) {
        ...
        pageCanvas.height = chunk;
        pageCanvas.getContext('2d').drawImage(canvas, 0, offsetY, canvas.width, chunk, 0, 0, canvas.width, chunk);
        if (!first) pdf.addPage();
        pdf.addImage(pageCanvas.toDataURL('image/png'), 'PNG', 0, 0, imgW, chunk / ratio);
        offsetY += sliceH;
        first = false;
      }
    }
